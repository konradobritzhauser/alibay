...
router.post("/removeItem",(req,res)=>{
    ...
    let itemId=req.body.itemId
    console.log('itemId', itemId)
    dbo.collection("carts").find({username:username}).toArray((err,result)=>{
...
router.post("/getCart",(req,res)=>{
    ...
    dbo.collection("carts").find({username:username}).toArray((err,result)=>{
        ...
        res.status(200).json({success:true,message:"cart retrieved",results:cart})
    })
})
